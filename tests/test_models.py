...
class TestCompartment:
    def test_default_mg_conc_is_zero(self):
        comp = Compartment('A', na_conc=150.0, cl_conc=120.0)
...
class TestCalculatePotentials:
    ...
    def asymmetric_junction(self):
        apical = Compartment('A', na_conc=50.0, cl_conc=45.0, mg_conc=0.2)
        basolateral = Compartment('D', na_conc=145.0, cl_conc=105.0, mg_conc=0.5)
    ...
    def test_symmetric_concentrations_give_zero_potential_monovalent(self, constants, temperature):
        apical = Compartment('A', na_conc=100.0, cl_conc=100.0, mg_conc=0.5)
        basolateral = Compartment('D', na_conc=100.0, cl_conc=100.0, mg_conc=0.5)
...
class TestCalculateFluxes:
    def test_symmetric_compartments_and_zero_potential_give_no_change(self, constants, temperature):
        apical = Compartment('A', na_conc=145.0, cl_conc=105.0, mg_conc=0.5)
        basolateral = Compartment('D', na_conc=145.0, cl_conc=105.0, mg_conc=0.5)
        junction = Junctions(apical, basolateral, p_na=10.0, p_cl=1.0, p_mg=3.0, voltage_clamp=0.0)

        changes = junction.calculate_fluxes(constants.R, temperature, constants.F, ('Na', 'Cl', 'Mg'), membrane_pot=0.0, dt=1e-4, volume=8e-20)
    ...
    def test_matches_manually_chained_physics_functions(self, constants, temperature):
        ...
        junction = Junctions(apical, basolateral, p_na=10.0, p_cl=1.0, p_mg=3.0)
        changes = junction.calculate_fluxes(R, T, F, ('Na', 'Cl', 'Mg'), membrane_pot, dt, volume)
    ...
    def test_flux_scales_linearly_with_permeability(self, constants, temperature):
        ...
        changes_1x = junction_1x.calculate_fluxes(R, T, F, ('Na',), membrane_pot=-0.02, dt=1e-4, volume=8e-20)
        changes_2x = junction_2x.calculate_fluxes(R, T, F, ('Na',), membrane_pot=-0.02, dt=1e-4, volume=8e-20)
