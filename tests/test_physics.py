...
class TestNernstEquation:
    @pytest.mark.parametrize("conc_apical,conc_basolateral", [(0, 100), (100, 0), (-5, 100), (100, -5)])
    def test_raises_for_nonpositive_concentration(self, constants, temperature, conc_apical, conc_basolateral):
        with pytest.raises(ValueError):
            nernst_equation(constants.R, temperature, constants.F, 1, conc_apical, conc_basolateral, "Na")
...
class TestGoldmannEquation:
    def test_matches_manual_formula(self, constants, temperature):
        ...
        P_Na, P_Cl = 10.0, 1.0
        na_apical, cl_apical, na_basolateral, cl_basolateral = 50.0, 45.0, 145.0, 105.0
        expected = (R * T / F) * math.log(
            (P_Na * na_apical + P_Cl * cl_basolateral) / (P_Na * na_basolateral + P_Cl * cl_apical)
        )
        actual = goldmann_equation(R, T, F, P_Na, P_Cl, na_apical, cl_apical, na_basolateral, cl_basolateral)
...
class TestDivalentMembranePotential:
    ...
    def test_reduces_to_goldmann_equation_when_p_mg_is_zero(self, constants, temperature):
        ...
        goldman = goldmann_equation(R, T, F, P_Na, P_Cl, na_apical, cl_apical, na_basolateral, cl_basolateral)
        divalent = divalent_membrane_potential(
            0.0, P_Na, P_Cl,
            999.0, 888.0,          # Mg concentrations are irrelevant when P_Mg=0
            na_basolateral, na_apical,  # bl, ap
            cl_basolateral, cl_apical,  # bl, ap
            R, T, F,
        )
...
class TestCalculateFlux:
    ...
    def compartments(self):
        apical = Compartment('apical', na_conc=150.0, cl_conc=0, volume=8e-20, mg_conc=0)
        basolateral = Compartment('basolateral', na_conc=15.0, cl_conc=0, volume=8e-20, mg_conc=0)
        return apical, basolateral

    def test_positive_drive_uses_apical_concentration(self, compartments):
        apical, basolateral = compartments
        # emf * p * z > 0
        flux = calculate_flux(emf=0.01, p=10.0, z=1, apical_comp=apical, basolateral_comp=basolateral, ion_name='Na')
        assert flux == pytest.approx(0.01 * 10.0 * 1 * apical.concentrations['Na'])

    def test_negative_drive_uses_basolateral_concentration(self, compartments):
        apical, basolateral = compartments
        # emf * p * z < 0
        flux = calculate_flux(emf=-0.01, p=10.0, z=1, apical_comp=apical, basolateral_comp=basolateral, ion_name='Na')
        assert flux == pytest.approx(-0.01 * 10.0 * 1 * basolateral.concentrations['Na'])

    def test_exact_zero_drive_uses_basolateral_concentration(self, compartments):
        """emf * p * z == 0 falls into the else/basolateral_comp branch -- the code comment
        'Threshold changed from 1 to 0!!!' flags this boundary as previously debugged."""
        apical, basolateral = compartments
        flux = calculate_flux(emf=0.0, p=10.0, z=1, apical_comp=apical, basolateral_comp=basolateral, ion_name='Na')
